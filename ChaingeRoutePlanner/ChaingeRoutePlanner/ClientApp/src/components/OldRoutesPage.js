        ...
        return (
            <Container>
                ...
                {routes.map((routes, index) =>
                    (index % 3 == this.state.selectedOption || 'All' == this.state.selectedOption) && this.state.date != null && <li className="list-group-item" key={index + 'routeID'}>
                        <Input onClick={() => this.handleRouteSelect(routes.routes, index)} className="form-check-input me-1 changie_checkBox" type="button" value={this.state.date + " / " + 'Bike ' + (1 + index % 3) } aria-label="..." style={{ width: '100%', height: '100%', backgroundColor: index == this.state.selectedR ? 'rgb(105, 219, 26)' : '', color: index == this.state.selectedR ? ' white' : '', fontWeight: 'bold'}} >
                        </Input>
